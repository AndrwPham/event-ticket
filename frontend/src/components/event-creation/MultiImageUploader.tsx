...
export const MultiImageUploader = ({ onFilesChange, label }: MultiImageUploaderProps) => {
    ...
    return (
        <div className="flex flex-col gap-2 w-full h-full">
            {/* IMPROVEMENT: This label is now a persistent title above the uploader box. */}
            <h4 className="font-semibold text-gray-700">{label}</h4>
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 flex-grow flex flex-col bg-white">
                <div className="flex-grow flex">
                    {files.length === 0 ? (
                        // FIX: The empty state label now fills the entire available space.
                        <label htmlFor="multi-image-upload-labeled" className="flex flex-col flex-grow items-center justify-center text-gray-400 cursor-pointer text-center h-full w-full hover:bg-gray-50 rounded-md">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 w-full">
                                ...
                                {showUploader && (
                                    <label htmlFor="multi-image-upload-labeled" className="flex items-center justify-center aspect-video bg-gray-100 rounded-md border-2 border-white hover:bg-gray-200 cursor-pointer shadow-md">
                ...
                {showUploader && (
                    <input type="file" id="multi-image-upload-labeled" multiple accept="image/*" onChange={handleFileChange} className="hidden" />
                )}
            </div>
        </div>
    );
